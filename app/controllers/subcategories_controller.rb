class SubcategoriesController < ApplicationController
  ...
  def index
    @subcategories = Subcategory.active_batch_scope.all.to_a

    @subcategories.sort! {
      |cur_subcategory| cur_subcategory.packages.exclude_unregistered_packages(cookies).length
    }

    @subcategories = @subcategories[0..99]
  end

  # GET /subcategories/1
  # GET /subcategories/1.json
  def show
    raw_params = params.deep_dup
    raw_params[:subcategory_id] = raw_params.delete :id
  ...
  private
    ...
    def set_subcategory
      @subcategory = Subcategory.custom_find(params[:id])
    end
